                                ...
                                <form>
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label for="nombrefull">* Título: <span id="maxsms"></span></label>
                                            <input value="<?php echo $rowGetAlert['alerta_title'] ?>" tabindex="1" type="text" class="form-control" id="titulo" placeholder="Titulo" maxlength="156">
                                        </div>
...
var myregions = {
	"region_all": "Todo el Pa&iacute;s",
	"panama": "Panam&aacute;",
	"panamaoeste": "Panam&aacute; Oeste",
	"panama-oeste": "Panam&aacute; Oeste",
	"colon": "Col&oacute;n",
	"cocle": "Cocl&eacute;",
	"veraguas": "Veraguas",
	"herrera": "Herrera",
	...
	"comarca-guna": "Guna Yala",
	"comarca-madugandi": "Madugand&iacute;",
	"comarca-wargandi": "Wargand&iacute;",
	"comarca-embera": "Ember&aacute;",
	"comarca-ngabe": "Ngobe Bugl&eacute;",
	"gunayala": "Guna Yala",
	"madugandi": "Madugand&iacute;",
	"wargandi": "Wargand&iacute;",
	"embera": "Ember&aacute;",
	"ngabe": "Ngobe Bugl&eacute;"
	};
...
$(document).ready(function () {
  ...
  let imagen1 = ""; // Version sin imagen
    ...
    $.ajax({
      ...
      success: (response) => {
        ...
        if (response.check == "success") {
			//
			$("#btn-edit-params").hide(); // Evita Volver a Salvar
			//$_POST['alertatitulo'],$_POST['alertatxt'],$_POST['color'],$_POST['fecha'],$_POST['region']);
			    var dataEnvio = {};
				var elregions = regions.split(",");
				var theregions = "<ul>";
				elregions.forEach(function(el){
					theregions = theregions + "<li>"+ myregions[el]+"</li>";
				});
				theregions = theregions + "</ul>";
				
				dataEnvio['alertatitulo'] = titulo;
				dataEnvio['alertatxt'] = descripcion;
				dataEnvio['color'] = color;
				dataEnvio['fecha'] = response.createddate;
				dataEnvio['region'] = theregions;
				dataEnvio['insertid'] = response.insertid;
				sendEmailReport(dataEnvio);
			//
				generaDocumento(dataEnvio);
          Swal.fire(
            avisodatabase,
            "operación exitosa!",
            "success"
...
function generaDocumento(dd){
	// Obtener el contenido del archivo del campo de texto con ID "contenido"
  

  // Hacer la solicitud AJAX utilizando el método POST
  $.ajax({
    url: "services/crear_file_alertas.php",
    type: "POST",
    data: dd,
    success: function(response) {
      // Mostrar un mensaje indicando que el archivo se creó correctamente
